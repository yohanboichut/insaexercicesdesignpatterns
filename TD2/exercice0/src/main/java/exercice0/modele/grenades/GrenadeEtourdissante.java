...
import exercice0.modele.poissons.DeplacementEtourdi;
import exercice0.modele.poissons.Poisson;

public class GrenadeEtourdissante extends Grenade{
    public GrenadeEtourdissante(int rayon, int x, int y) {
        super(rayon, x, y);
    }
    ...
    public void impact(Poisson poisson) {
        ...
        if (distancePoissonImpact <= getRayon()) {
            poisson.setComportementDeplacement(new DeplacementEtourdi(poisson));
        }
    }
}
